...
__all__ = ['PreActResNet', 'preactresnet18', 'preactresnet18smaller', 'preactresnet18smaller3Layers', 'preactresnet34',
...
class PreActResNet(nn.Module):
    ...
    def __init__(self, block, layers, nchannels=3, nfilters=64,
                 nclasses=1000, r=2, normalize_output=False):
...
class PreActResNetSmaller3Layers(nn.Module):

    def __init__(self, block, layers, nchannels=3, nfilters=64,
                 nclasses=1000, r=2, normalize_output=False):
        super(PreActResNetSmaller3Layers, self).__init__()
        self.nclasses = nclasses
        self.r = r
        self.nchannels = nchannels
        self.nfilters = nfilters
        ...
        self.dr = nn.Dropout(0.5)
        self.fc = nn.Linear(4 * self.nfilters * block.expansion, self.r, bias=False)
        self.bn = nn.BatchNorm1d(self.r)
        self.normalize_output = normalize_output
...
class PreActResNetSmaller(nn.Module):

    def __init__(self, block, layers, nchannels=3, nfilters=64,
                 nclasses=1000, r=2, normalize_output=False):
        super(PreActResNetSmaller, self).__init__()
        self.nclasses = nclasses
        self.r = r
        self.nchannels = nchannels
        self.nfilters = nfilters
        ...
        self.dr = nn.Dropout(0.5)
        self.fc = nn.Linear(2 * self.nfilters * block.expansion, self.r, bias=False)
        self.bn = nn.BatchNorm1d(self.r)
        self.normalize_output = normalize_output
...
class PreActResNetEndDec(nn.Module):
    ...
    def __init__(self, block, layers, nchannels=3, nfilters=64,
        ...
        self.decoder = nn.Sequential(
            nn.Linear(r, 50),
            nn.ReLU(),
            nn.Linear(50, nclasses, bias=True),
        )
...
def preactresnet18smaller(**kwargs):
    """Constructs a PreActResNet-18 model.
    """
    model = PreActResNetSmaller(PreActBasicBlock, [2, 2, 2, 2], **kwargs)
    return model

def preactresnet18smaller3Layers(**kwargs):
    """Constructs a PreActResNet-18 model.
    """
    model = PreActResNetSmaller3Layers(PreActBasicBlock, [2, 2, 2, 2], **kwargs)
    return model
