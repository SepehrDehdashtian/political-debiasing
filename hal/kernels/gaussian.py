        ...
        if y is None:
            if self.sigma is None:
                ...
                if (dist_u > 0).sum():
                    ...
                    sigma = 1.0
                # sigma =  0.5 * torch.gt(torch.triu(dist, diagonal=1), 0).median().detach()
            else:
                dist = torch.cdist(x, x, p=2) ** 2
                sigma = self.sigma
        else:
...
class RFFGaussian:
    ...
    def _calc_w_b(self, x):
        ...
        if self.sigma is None:
            ...
            # sigma_x =  0.5 * torch.gt(torch.triu(dist, diagonal=1), 0).to(dtype=x.dtype).median()
        ...
        # sigma_x = torch.eye(dim_x, device=x.device) / (sigma* math.sqrt((2 * math.pi) ** dim_x))
