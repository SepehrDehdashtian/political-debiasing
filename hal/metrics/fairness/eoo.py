...
class EOO(tm.Metric): # TODO: Complete this ...
    def __init__(self,
        ...
        self.add_state("total", default=torch.zeros((num_sensitive_att, num_y_classes, num_s_classes)), dist_reduce_fx=None)
        self.add_state("n_prob", default=torch.zeros((num_sensitive_att, num_y_classes, num_s_classes)), dist_reduce_fx=None)
        
        self.num_s_classes     = num_s_classes
        self.num_y_classes     = num_y_classes
        self.num_sensitive_att = num_sensitive_att

    def update(self, yhat, y, list_s):
            ...
            for i in range(self.num_sensitive_att): 
                ...
                for y in range(self.num_y_classes):
                    for s_0 in range(self.num_s_classes):
                        ...
                        n_prob[i, y, s_0] = len(pred_0[pred_0 == y]) # Number of samples with yhat = y & s = s_0 { P(yhat=y, s=s_0) * #Batch_size}
               

               
        else: # If we have only one sensitive attribute
            s = list_s[:]
            for y_0 in range(self.num_y_classes):
                for s_0 in range(self.num_s_classes):
                    ...
                    # n_prob[0, y, s_0] = len(pred_0[pred_0 == y]) # Number of samples with yhat = y & s = s_0 { P(yhat=y, s=s_0) * #Batch_size}

                    mask_s = (s == s_0)
                    mask_y = (y == y_0)

                    mask_pred = (pred == 1)
                    n_prob[0, y_0, s_0] = sum(mask_s * mask_y * mask_pred)
                    total[0, y_0, s_0]  = sum(mask_s * mask_y)
    ...
    def compute(self):
        ...
        prob = prob.transpose(1,0).squeeze().to(device=self.total.device)
        ...
        for y_class, prob_y in enumerate(prob):
            ...
            var = prob_y.var()
            max, avg = self._cross_diff(prob_y)
            avg_list.append(avg)
            out.append({'var': var, 'max': max, 'avg': avg})
            sum_var += var
            if max > total_max:
                total_max = max
        ...
        total_avg_max = torch.Tensor(avg_list).max()
            
        output['EOO_m_max' ]     = total_max
        output['EOO_m_var' ]     = np.sqrt(total_var.cpu())
        output['EOO_m_avg' ]     = total_avg.cpu()
        output['EOO_m_avg_max' ] = total_avg_max.cpu()
    ...
    def _cross_diff(self, x):
        ...
        # sqx = torch.Tensor(len(x)*[x.cpu().numpy().tolist()]).to(device=self.total.device) # Repeat x in rows in order to make a square tensor
        
        sqx = x.repeat(len(x), 1).to(device=self.total.device) # Repeat x in rows in order to make a square tensor
        
        cross_diff_tensor = abs(sqx - sqx.transpose(1,0))

        max = cross_diff_tensor.max()

        avg = cross_diff_tensor.mean()
